...
class ServerWithQueue(mesa.Agent):
    ...
    def step(self):
        ...
        if self.passenger_in_service:
            if self.passenger_in_service.service_time_remaining > 1: # Edited, was 0
...
class LuggagePlacing(ServerWithQueue):
    def __init__(self, model, name, distribution):
        super().__init__(model, name, distribution)
        self.num_luggage_on_belt = 0
        # self.belt_capacity = belt_capacity
        # self.passengers_with_luggage_on_belt = []
        self.num_luggage_moved = 0
        self.typical_luggage_time = 10 / 0.5  # seconds. Assume a fixed time to complete conveyer belt
    ...
    def step(self):
        ...
        if self.passenger_in_service:
            if self.passenger_in_service.service_time_remaining > 1: # Edited, was 0
                ...
                self.passenger_in_service.bool_waiting_for_luggage = True
                self.passenger_in_service.luggage_time_remaining = self.typical_luggage_time
                self.model.passengers_with_luggage_on_belt.insert(0, self.passenger_in_service)

                # Release passenger from server
                self.release(self.passenger_in_service)
                self.total_served += 1

        self.num_in_queue = len(self.queue)  # Update during time step to use here

        ## Then, fill up new places from queue
        if self.num_in_queue > 0:
            if not self.passenger_in_service:
                self.dequeue() # Dequeue next passenger if none are in service
            else:
                pass # No places available
        else:
            # Case: nobody in queue or server
            pass

        ## Move luggage by adjusting time remaining
        for passenger in self.model.passengers_with_luggage_on_belt:
            if passenger.luggage_time_remaining > 0:
                passenger.luggage_time_remaining -= 1
            else:
                logger.info(
                    f"t = {self.model.schedule.time}: {self.name}: Luggage of passenger {passenger.unique_id} has reached end of belt.")
                passenger.bool_waiting_for_luggage = False
                passenger.bool_luggage_at_end = True
                self.model.passengers_with_luggage_on_belt.remove(passenger) # Passengers can remove luggage parallel and in any order
                self.num_luggage_moved += 1

        ## Output of tick
        self.num_in_service = 1 if bool(self.passenger_in_service) else 0
        self.num_in_queue = len(self.queue)
        self.num_luggage_on_belt = len(self.model.passengers_with_luggage_on_belt)
        # logger.info(f"t = {self.model.schedule.time}, {self.name} has queue: {self.queue}")
...
class PassengerScan(ServerWithQueue):
    def __init__(self, model, name, distribution):
        super().__init__(model, name, distribution)

    def release(self, passenger): # Change release function
        self.passenger_in_service = None # Reset
        passenger.service_time_remaining = 0 # Reset to avoid confusion

        ## Check if agent needs a manual check
        if passenger.bool_needs_manual_check:
            logger.info(
                f"t = {self.model.schedule.time}: {self.name} releases passenger {passenger.unique_id} to manual check.")
            passenger.go_to_next_action()
        else:
            # Advance passenger's action by 1, to skip manual check
            passenger.current_action = next(passenger.actions)
            logger.info(
                f"t = {self.model.schedule.time}: {self.name} releases passenger {passenger.unique_id} to luggage pick up")
            passenger.go_to_next_action()

        ## Send message that an agent departs from this server.
        self.model.messages.append('departure ' + self.name)

class ServerWithoutQueue(mesa.Agent):
    """
    A server with no queue and infinite capacity
    """
    def __init__(self, model, name, distribution):
        super().__init__(self, model)
        self.model = model
        ...
        ## Service / queue variables
        self.queue = [] # Note: there is no queue
        self.num_in_queue = None
        self.num_in_service = 0
        self.passengers_in_service = [] # Now a list, capacity > 1
    ...
    def enter(self, passenger):
        self.enter_service(passenger) # Redirect

    def enter_service(self, passenger):
        self.passengers_in_service.insert(0, passenger)
        passenger.service_time_remaining = self.distribution.rvs()
    ...
    def release(self, passenger):
        self.passengers_in_service.remove(passenger)
        passenger.service_time_remaining = 0 # Reset to avoid confusion
        passenger.go_to_next_action()
        ## Send message that an agent departs from this server.
        self.model.messages.append('departure ' + self.name)
    ...
    def do_service(self):
        for passenger in self.passengers_in_service:
            if passenger.service_time_remaining > 0:
                passenger.service_time_remaining =- 1
                self.total_service_time += 1
            else: # Passenger is done
                self.release(passenger)
                self.total_served += 1
                logger.info(f"t = {self.model.schedule.time}: {self.name}: Releases passenger {passenger.unique_id}")

    def step(self):
        ## Advance or release passengers in service
        self.do_service()

        # Logging
        self.num_in_service = len(self.passenger_in_service)
        if self.num_in_service > 0:
            self.total_service_time += 1 # Count times where any passengers are in service
        self.utilization = self.total_service_time / self.model.schedule.time if self.model.schedule.time>0 else 0

class LuggagePickup(ServerWithoutQueue):
    def __init__(self, model, name, distribution):
        super().__init__(model, name, distribution)
        self.passengers_waiting_for_luggage = []
        self.num_waiting_for_luggage = 0

    def enter(self, passenger):
        ## Only enter service when luggage has reached end of belt.
        if passenger.bool_waiting_for_luggage:
            self.passengers_waiting_for_luggage.append(passenger)
            # self.total_time_waiting_for_luggage += 1
            passenger.time_waiting_for_luggage += 1
            logger.info(
                f"t = {self.model.schedule.time}: {self.name}: Passenger {self.unique_id} is waiting for luggage. Time waiting: {passenger.time_waiting_for_luggage}")
        else:
            self.enter_service(passenger)
    ...
    def release(self, passenger):
        self.passengers_in_service.remove(passenger)
        passenger.service_time_remaining = 0 # Reset to avoid confusion
        passenger.bool_luggage_at_end = 0 # Indicate that luggage has been picked up

        ## Gather data from passenger
        passenger.time_of_termination = self.model.schedule.time
        throughput_time = passenger.time_of_termination - passenger.time_of_creation
        self.model.lst_throughput_time.append(throughput_time)

        self.model.lst_time_waiting_for_luggage.append(passenger.time_waiting_for_luggage)
        self.model.lst_time_luggage_at_end.append(passenger.time_luggage_at_end)

        ## Terminate passenger from system
        logger.info(f"t = {self.model.schedule.time}: {self.name}: Terminating passenger {passenger.unique_id}, with throughput time {throughput_time}")
    ...
    def step(self):
        for passenger in self.passengers_waiting_for_luggage:
            if not passenger.bool_waiting_for_luggage:
                self.enter_service(passenger)
                self.passengers_waiting_for_luggage.remove(passenger)
            else:
                logger.info(
                    f"t = {self.model.schedule.time}: {self.name}: Passenger {self.unique_id} is waiting for luggage. Time waiting: {passenger.time_waiting_for_luggage}")
                # self.total_time_waiting_for_luggage += 1
        ...
        ## Logging
        self.num_in_service = len(self.passengers_in_service)
        self.num_waiting_for_luggage = len(self.passengers_waiting_for_luggage)



        ## Do service for passenger in service
...
class Passenger(mesa.Agent):
    ...
    def step(self):
        ...
        else: # Passenger is doing somthing in a server
            pass

        ## Count how long passenger's luggage is waiting
        if self.bool_luggage_at_end:
            self.time_luggage_at_end += 1
            logger.debug(
                f"Luggage of passenger {self.unique_id} at end of belt. Waiting for {self.time_luggage_at_end} seconds.")
...
distributions = {
    ...
    PassengerAction(4): stats.uniform(loc=120, scale=180), # Manual check
...
class Airport(mesa.Model):
    def __init__(self, seed=1, distributions=distributions, logger_level=logging.INFO, show_messages=True):
        ...
        self.schedule = RandomActivation(self) # For passengers and PassengerSource
        self.schedule2 = RandomActivation(self) # For servers

        ## Make servers and add to scheduler2
        ...
        self.luggage_placing = LuggagePlacing(self, 'Luggage placing',
                                               distributions[PassengerAction(2)])
        self.passenger_scan = PassengerScan(self, 'Passenger scan', distributions[PassengerAction(3)])
        self.manual_check = ServerWithQueue(self, 'Manual check', distributions[PassengerAction(4)])
        self.luggage_pickup = LuggagePickup(self, 'Luggage pickup', distributions[PassengerAction(5)])

        self.schedule2.add(self.passport_check)
        self.schedule2.add(self.luggage_placing)
        self.schedule2.add(self.passenger_scan)
        self.schedule2.add(self.manual_check)
        self.schedule2.add(self.luggage_pickup)
        ...
        ## Luggage
        self.passengers_with_luggage_on_belt = [] # Init. list
        self.num_luggage_at_end_of_belt = 0
        ...
        self.lst_throughput_time = []
        self.lst_time_luggage_at_end = []
        self.lst_time_waiting_for_luggage = []
    ...
    def step(self):
        ...
        ## Count luggage at end of belt
        self.num_luggage_at_end_of_belt = 0
        for passenger in self.passengers_with_luggage_on_belt:
            if passenger.bool_luggage_at_end:
                self.num_luggage_at_end_of_belt += 1
...
if __name__ == "__main__":
    model1 = Airport(seed=1, logger_level=logging.INFO, show_messages=True)
