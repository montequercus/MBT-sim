...
"""
This model has some subtle faults regarding activation order.
"""
...
class ServerWithQueue(mesa.Agent):
    ...
    def step(self):
        ...
        if self.passenger_in_service:
            if self.passenger_in_service.service_time_remaining > 0:
...
class Airport(mesa.Model):
    def __init__(self, seed=1, distributions=distributions, logger_level=logging.INFO, show_messages=True):
        ...
        self.schedule.add(self.passport_check)
        self.schedule.add(self.luggage_placing)
...
if __name__ == "__main__":
    for i in range(0, 20):
        ...
        # print(model.passport_check.num_in_queue)
        # print(model.passport_check.num_in_service)
...
# class Airport(mesa.Model):
    # def __init__(self, seed=1, distributions=dict_distributions, bool_print_logs=False):
